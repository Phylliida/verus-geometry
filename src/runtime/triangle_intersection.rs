...
pub fn triangles_intersect_strict_exec<R: RuntimeOrderedFieldOps<V>, V: OrderedField>(
    a1: &RuntimePoint3<R, V>, b1: &RuntimePoint3<R, V>, c1: &RuntimePoint3<R, V>,
    a2: &RuntimePoint3<R, V>, b2: &RuntimePoint3<R, V>, c2: &RuntimePoint3<R, V>,
) -> (out: bool)
    requires
        a1.wf_spec(), b1.wf_spec(), c1.wf_spec(),
        a2.wf_spec(), b2.wf_spec(), c2.wf_spec(),
    ensures
        out == triangles_intersect_strict::<V>(
            a1.model@, b1.model@, c1.model@, a2.model@, b2.model@, c2.model@,
        ),
{
    ...
    if t1_all_above || t1_all_below {
        proof {
            lemma_t1_separated_no_t1_edges::<V>(
                a1.model@, b1.model@, c1.model@, a2.model@, b2.model@, c2.model@,
            );
        }
    }
    ...
    if (t1_all_above || t1_all_below) && (t2_all_above || t2_all_below) {
        proof {
            lemma_both_separated_no_intersection::<V>(
                a1.model@, b1.model@, c1.model@, a2.model@, b2.model@, c2.model@,
            );
        }
        return false;
    ...
    if segment_triangle_intersects_strict_exec(a1, b1, a2, b2, c2) { return true; }
    if segment_triangle_intersects_strict_exec(b1, c1, a2, b2, c2) { return true; }
    if segment_triangle_intersects_strict_exec(c1, a1, a2, b2, c2) { return true; }
    if segment_triangle_intersects_strict_exec(a2, b2, a1, b1, c1) { return true; }
    if segment_triangle_intersects_strict_exec(b2, c2, a1, b1, c1) { return true; }
    if segment_triangle_intersects_strict_exec(c2, a2, a1, b1, c1) { return true; }
    false
}
